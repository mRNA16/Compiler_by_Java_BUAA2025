...
public class IrDeclVisitor {
    ...
    public static void visitConstDef(ConstDef constDef, boolean isGlobal) {
        ...
        if (!(symbol instanceof VariableSymbol varSymbol))
            throw new RuntimeException("Symbol is not a variable" + constName);
        IrType irType = IrTypeConverter.symbolType2IrType(varSymbol.getType(), varSymbol.getElementCount());

        if (isGlobal) {
            IrGlobalValue irGlobalValue = IrBuilder.getNewGlobalValueIr(new IrPointerType(irType),
                    getConstInitValue(varSymbol));
    ...
    public static void visitVarDef(VarDef varDef, boolean isGlobal) {
        ...
        if (!(symbol instanceof VariableSymbol varSymbol))
            throw new RuntimeException("Symbol is not a variable " + varName);
        IrType irType = IrTypeConverter.symbolType2IrType(varSymbol.getType(), varSymbol.getElementCount());
                ...
                if (!varSymbol.isArray()) {
                    ...
                    if (varDef.hasInitVal() && exps != null) {
                        ...
                        for (int i = 0; i < size; i++) {
                            IrValue irValue;
                            if (i < exps.size()) {
                                irValue = IrExpVisitor.visitExp(exps.get(i));
                            } else {
                                irValue = new IrConstInt(0);
                            }
                            GepInstr gepInstr = new GepInstr(allocateInstr, new IrConstInt(i));
